...
- **Teardown #6 PUBLISHED 2026-07-20 (commit `bbcae9a`, pushed, live, 200 verified):** `bearliu.com/blog/google-health-extraction-by-design` — *Google Health: Extraction by Design*. Again **manual, not via the porter**, same reason as #5 (7 figures + inline links). Files: `content/blog/google-health-extraction-by-design.mdx` + `public/blog/google-health-extraction-by-design/fig-0…6.webp`. **Image naming changed on Bear's call:** `fig-N.webp` matching the in-text Fig numbers, replacing #5's opaque `01–08` sequence. Site and vault numbering are 1:1 here (no offset, unlike #5). Source: vault `6. Google Health/Teardown Report - Google Health (FDP) v2.md`; published version drops footnotes + Sources and converts 12 cited sources to inline links, adds one link the vault version had in footnotes but never used in body (Android Authority hallucination piece), and compresses the vault's frontmatter thesis blockquote into a paragraph at the end of "Why Google Health" (the web layout has no equivalent block). Blog total now **6 teardowns**. **New for this one: all figures are real screenshots + Bear's Freeform handwriting**, not generated vector diagrams — see `20_Projects/Product Teardown/Teardown Reports - Image Plan.md` §6 for the full record and two transferable lessons.
- **Teardown #5 PUBLISHED 2026-06-08 (commit `f86c4e9`, live, 200 verified):** `bearliu.com/blog/apple-health-infrastructure-by-design` — *Apple Health: Infrastructure by Design*. Done **manually, not via the porter** because this one carries 8 generated figures (vector diagrams + annotated screenshots) and inline source links. Files: `content/blog/apple-health-infrastructure-by-design.mdx` + `public/blog/apple-health-infrastructure-by-design/01–08.webp`. Published version drops footnotes/Sources (matching the other 4) and converts ~20 key data points to inline external links. Lens 5 has no figure yet (site Fig numbering stays sequential 0–7). Blog total now **5 teardowns**. This counts toward the "weekly cadence proven" precondition for resuming 14.4.
- **Teardown #5 CONTENT SWAPPED v1 → v2, 2026-07-21 (committed locally, NOT yet pushed — live site still serves v1):** the site had been running the original v1 (six `Lens 1–6` headings) while the vault's v2 rewrite (2026-06-19, plainer language for founders, sections renumbered) never shipped. Swapped so #5 and #6 share one structure (numbered sections + *Where this could be wrong* + *When to copy* + *What this means*), which the Apple-vs-Google comparison distribution depends on. 25 footnotes converted to inline links per site convention, Sources section stays off-site, all 8 image paths and positions unchanged (`01–08.webp`, site captions still sequential Fig 0–7 because Lens 5 has no figure). `npm run build` clean, exit 0, 13/13 static pages. Driven by `ClaudeCode/docs/superpowers/specs/2026-07-21-health-teardown-distribution-design.md` §8. **Still missing: `apple-health-fig-6`** (the Data Sources arbitration figure); if Bear generates it, insert after `06.webp` and renumber the two figures below it.
...
- To re-port/add a teardown: `node scripts/port-teardown.mjs --report "<vault path>" --slug <slug> --date <YYYY-MM-DD> --tags "teardown,..."` then commit. **But note the porter has now been bypassed twice in a row** (Apple Health #5 on 2026-06-08, Google Health #6 on 2026-07-20), both times because the report carried figures + inline source links, which the porter does not handle. If teardowns keep shipping with figures (they will), either extend the porter to cover the figure/`<Figure>` + footnote→inline-link conversion, or retire it and treat manual conversion as the standard path. Decide this before #7 rather than bypassing a third time.
- Open item from Phase 14 answered in practice: "publish the blog publicly before Claude Design's rich visual layer lands, or hold" — **published**, twice (#5, #6). No longer blocking.
